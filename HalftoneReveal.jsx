// HalftoneReveal — adapted from React Bits (reactbits.dev), JS + CSS variant.
// Deps: ogl (loaded as window.ogl via an ES-module shim in the DC <helmet>).
// Classic script: React is injected globally; returns via React.createElement.
// Exposes window.HalftoneReveal.

const { useRef, useEffect } = React;

const HALFTONE_CSS = `
.halftone-reveal { position: relative; width: 100%; height: 100%; overflow: hidden; touch-action: none; cursor: crosshair; }
.halftone-reveal canvas { display: block; width: 100%; height: 100%; }
`;
(function injectCss() {
  if (typeof document === 'undefined' || document.getElementById('halftone-reveal-css')) return;
  const el = document.createElement('style');
  el.id = 'halftone-reveal-css';
  el.textContent = HALFTONE_CSS;
  document.head.appendChild(el);
})();

const DEFAULT_SRC = 'images/halftone-quote.png';
...
const HalftoneReveal = ({
  ...
  const _n = (v, d) => { const x = typeof v === 'number' ? v : parseFloat(v); return Number.isFinite(x) ? x : d; };
  dotSize = _n(dotSize, 1); dotDensity = _n(dotDensity, 128); angle = _n(angle, 22);
  contrast = _n(contrast, 1.22); revealRadius = _n(revealRadius, 0.32); edge = _n(edge, 0.68);
  follow = _n(follow, 0.32); idleReveal = _n(idleReveal, 0);
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      const program = new Program(gl, { vertex: HT_VERT, fragment: HT_FRAG, uniforms });
    ...
    if (window.ogl) start();
      ...
      const poll = setInterval(() => { if (window.ogl) { clearInterval(poll); start(); } }, 60);
      teardown = () => clearInterval(poll);
    }
  ...
  return React.createElement('div', {
    ...
    style: Object.assign({ borderRadius }, style)
  });
};

if (typeof window !== 'undefined') window.HalftoneReveal = HalftoneReveal;
if (typeof module !== 'undefined' && module.exports) module.exports = { HalftoneReveal, default: HalftoneReveal };
